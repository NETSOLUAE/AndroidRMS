...
public class DatabaseManager {
    ...
    private static final String DATABASE_NAME = "RMSDATA.db";
    ...
    public void createDb() {
        ...
        db.execSQL("CREATE Table EB_POLICY_DETAIL_GROUP(MEMBER_NAME TEXT, MEMBER_ID TEXT, RELATIONSHIP_TYPE TEXT)");
        db.execSQL("CREATE Table EB_POLICY_DETAIL_CHILD(MEMBER_ID TEXT, STAFF_ID TEXT, COMPANY_NAME TEXT, POLICY_REF TEXT, START_DATE TEXT, END_DATE TEXT)");
        ...
        // To Create FAQ Tables
        db.execSQL("CREATE Table FAQ_QUESTIONS(ID TEXT PRIMARY KEY, TITLE TEXT)");
        db.execSQL("CREATE Table FAQ_ANSWERS(ID TEXT, INDEXS TEXT, DESCRIPTION TEXT)");
    ...
    public void clearEbPolicyDetails() {
        openConnection();
        // To clear transaction Tables
        db.execSQL("DELETE FROM EB_POLICY_DETAIL_GROUP");
        db.execSQL("DELETE FROM EB_POLICY_DETAIL_CHILD");
        closeConnection();
    }
    ...
    public void insertEbPolicyDetailsGroup(EbPolicyDetailGroup _ebPolicyDetailGroup) {
        openConnection();
        try {
            String qryString = "Insert into EB_POLICY_DETAIL_GROUP(MEMBER_NAME, MEMBER_ID, RELATIONSHIP_TYPE) Values ('"
                    + _ebPolicyDetailGroup.getMemberName()
                    + "', '"
                    + _ebPolicyDetailGroup.getmemberID()
                    + "', '"
                    + _ebPolicyDetailGroup.getRelationship() + "')";
            db.execSQL(qryString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<EbPolicyDetailGroup> getEbPolicyDetailsGroup() {
        openConnection();
        String query = "SELECT * FROM EB_POLICY_DETAIL_GROUP";
        Cursor c = db.rawQuery(query, null);

        ArrayList<EbPolicyDetailGroup> ebPolicyDetailGroupList = new ArrayList<EbPolicyDetailGroup>();

        if ((c != null) && (c.getCount() > 0)) {
            if (c.moveToFirst()) {
                do {
                    EbPolicyDetailGroup ebPolicyDetailGroup = new EbPolicyDetailGroup();
                    ebPolicyDetailGroup.setMemberName(c.getString(c
                            .getColumnIndex("MEMBER_NAME")));
                    ebPolicyDetailGroup.setmemberID(c.getString(c
                            .getColumnIndex("MEMBER_ID")));
                    ebPolicyDetailGroup.setRelationship(c.getString(c
                            .getColumnIndex("RELATIONSHIP_TYPE")));
                    ebPolicyDetailGroupList.add(ebPolicyDetailGroup);
                } while (c.moveToNext());
            }
        }
        c.close();
        closeConnection();
        return ebPolicyDetailGroupList;
    }

    public void insertEbPolicyDetailsChild(EbPolicyDetailChild _ebPolicyDetailChild) {
        openConnection();
        try {
            String qryString = "Insert into EB_POLICY_DETAIL_CHILD(MEMBER_ID, STAFF_ID, COMPANY_NAME, POLICY_REF, START_DATE, END_DATE) Values ('"
                    + _ebPolicyDetailChild.getMemberID()
                    + "', '"
                    + _ebPolicyDetailChild.getStaffId()
                    + "', '"
                    + _ebPolicyDetailChild.getCompanyName()
                    + "', '"
                    + _ebPolicyDetailChild.getPolicyRef()
                    + "', '"
                    + _ebPolicyDetailChild.getStartDate()
                    + "', '"
                    + _ebPolicyDetailChild.getEndDate() + "')";
            db.execSQL(qryString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public ArrayList<EbPolicyDetailChild> getEbPolicyDetailsChild(String memberID) {
        openConnection();
        String query = "SELECT * FROM EB_POLICY_DETAIL_CHILD WHERE MEMBER_ID = '" + memberID + "'";
        Cursor c = db.rawQuery(query, null);

        ArrayList<EbPolicyDetailChild> ebPolicyDetailChildList = new ArrayList<EbPolicyDetailChild>();

        if ((c != null) && (c.getCount() > 0)) {
            if (c.moveToFirst()) {
                do {
                    EbPolicyDetailChild ebPolicyDetailChild = new EbPolicyDetailChild();
                    ebPolicyDetailChild.setMemberID(c.getString(c
                            .getColumnIndex("MEMBER_ID")));
                    ebPolicyDetailChild.setStaffId(c.getString(c
                            .getColumnIndex("STAFF_ID")));
                    ebPolicyDetailChild.setCompanyName(c.getString(c
                            .getColumnIndex("COMPANY_NAME")));
                    ebPolicyDetailChild.setPolicyRef(c.getString(c
                            .getColumnIndex("POLICY_REF")));
                    ebPolicyDetailChild.setstartDate(c.getString(c
                            .getColumnIndex("START_DATE")));
                    ebPolicyDetailChild.setEndDate(c.getString(c
                            .getColumnIndex("END_DATE")));
                    ebPolicyDetailChildList.add(ebPolicyDetailChild);
                } while (c.moveToNext());
            }
        }
        c.close();
        closeConnection();
        return ebPolicyDetailChildList;
    }
    ...
    public void clearAccountSummay() {
        openConnection();
        // To clear transaction Tables
    ...
    public void clearAccountSummaryLines() {
        openConnection();
        // To clear transaction Tables
    ...
    /**
     * Start of FAQ Table Transaction
     *
     */
    ...
    public void clearFaq() {
        openConnection();
        // To clear transaction Tables
        db.execSQL("DELETE FROM FAQ_QUESTIONS");
        db.execSQL("DELETE FROM FAQ_ANSWERS");
        closeConnection();
    }

    public int getFaqQuestionsCount() {
        openConnection();
        String countQuery = "SELECT ID FROM FAQ_QUESTIONS";
        SQLiteDatabase db = openHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int cnt = cursor.getCount();
        cursor.close();
    ...
    public int getFaqAnswerCount(int index) {
        openConnection();
        String countQuery = "SELECT DESCRIPTION FROM FAQ_ANSWERS WHERE INDEXS = " + index;
        SQLiteDatabase db = openHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int cnt = cursor.getCount();
        cursor.close();
    ...
    public void insertFaqQuestions(FAQQuestions _faqQuestions) {
        openConnection();
        try {
//            String qryString = "Insert into FAQ_QUESTIONS(ID, TITLE) Values ('"
//                    + _faqQuestions.getID()
//                    + "', '"
//                    + _faqQuestions.getTitle() + "')";
//            db.execSQL(qryString);

            String insert_info = "INSERT INTO FAQ_QUESTIONS(ID, TITLE) VALUES (?,?)";
            SQLiteStatement stmt = db.compileStatement(insert_info);
            stmt.bindString(1, ""+_faqQuestions.getID());
            stmt.bindString(2, ""+_faqQuestions.getTitle());
            stmt.execute();
    ...
    public FAQQuestions getFaqQuestions(int index) {
        openConnection();
        String query = "SELECT * FROM FAQ_QUESTIONS WHERE ID = " + index;
        Cursor c = db.rawQuery(query, null);
        FAQQuestions faqQuestions = new FAQQuestions();
        if ((c != null) && (c.getCount() > 0)) {
            if (c.moveToFirst()) {
                faqQuestions.setID(c.getString(c.
                        getColumnIndex("ID")));
                faqQuestions.setTitle(c.getString(c.
    ...
    public void insertFaqAnswers(FAQAnswers _faqAnswers) {
        openConnection();
        try {
            String qryString = "Insert into FAQ_ANSWERS(ID, INDEXS, DESCRIPTION) Values ('"
                    + _faqAnswers.getID()
                    + "', '"
                    + _faqAnswers.getIndex()
                    + "', '"
                    + _faqAnswers.getDescription() + "')";
            db.execSQL(qryString);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public FAQAnswers getFaqAnswers(String index) {
        openConnection();
        String query = "SELECT * FROM FAQ_ANSWERS WHERE INDEXS = " + index;
        Cursor c = db.rawQuery(query, null);
        FAQAnswers faqAnswers = new FAQAnswers();
        if ((c != null) && (c.getCount() > 0)) {
            if (c.moveToFirst()) {
                faqAnswers.setID(c.getString(c
                        .getColumnIndex("ID")));
                faqAnswers.setIndex(c.getString(c
                        .getColumnIndex("INDEXS")));
                faqAnswers.setDescription(c.getString(c
